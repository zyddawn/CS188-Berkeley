...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.q_dic[(state, action)] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * sample
...
class PacmanQAgent(QLearningAgent):
    ...
    def getAction(self, state):
        ...
        method.
        """
        action = QLearningAgent.getLegalActions(self, state)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for key in self.featExtractor.getFeatures(state, action):
            self.weights[key] = self.weights[key] + self.alpha * diff * self.featExtractor.getFeatures(state, action)[key]
