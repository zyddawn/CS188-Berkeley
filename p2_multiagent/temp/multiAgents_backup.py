...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        alpha, beta = pos_infty, neg_infty
        
        def max_value(cur_state, depth, agent_id, alpha, beta):
            assert agent_id == 0, "max_value should only be applied on pacman (agent_id=0)"
            stop_flag = depth > self.depth
            win_flag = cur_state.isWin()
            lose_flag = cur_state.isLose()
            if stop_flag or win_flag or lose_flag:
                return self.evaluationFunction(cur_state), alpha, beta     # a value
            
            succ_actions = cur_state.getLegalActions(agent_id)
            val = neg_infty
            for act in succ_actions:
            	act_state_val, _, _ = min_value(cur_state.generateSuccessor(agent_id, act), depth, \
                                        agent_id+1, alpha, beta)
                val = max(val, act_state_val)
                if val > beta: 
        ...
        def min_value(cur_state, depth, agent_id, alpha, beta):
            ...
            succ_actions = cur_state.getLegalActions(agent_id)
            val = pos_infty
            if 0<agent_id<total_agents-1:
            	#print "min_value", agent_id, depth
                max_or_min_v = min_value 
            else:
                max_or_min_v, depth = max_value, depth+1
                #print "max_value", agent_id, depth
            for act in succ_actions:
            	act_state_val, _, _ = max_or_min_v(cur_state.generateSuccessor(agent_id, act), depth, \
                                            (agent_id+1)%total_agents, alpha, beta)
                val = min(val, act_state_val)
                if val < alpha: 
                	return val, alpha, beta
                beta = min(val, beta)
            return val, alpha, beta
        ...
        for act in gameState.getLegalActions(0):
            val = max(val, min_value(gameState.generateSuccessor(0, act), 1, 1, alpha, beta))
            if alpha == neg_infty:
                alpha, best = val, act
            else:
                alpha, best = max(val, alpha), act if val > alpha else best
        '''
        return max(gameState.getLegalActions(0), key=lambda act: min_value(gameState.generateSuccessor(0, act), 1, 1, alpha, beta)[0])
